pipeline {
    ...
    parameters {
        string(name: 'PREDICTION_ARGS', defaultValue: '--label num_not_done_points+num_removed_points+num_added_points --binary --roll-sprints --roll-labels --replace-na --model dnn --test-interval 200 --num-epochs 1000 --keep-index num_story_points --time time --time-size 105 --log WARNING', description: 'Prediction arguments')
